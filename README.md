...
- `/pethroom-friends`의 `건강지표 대시보드` 구간은 이미지 원본 비율을 유지하고, 데스크톱/모바일에서는 수동 줄바꿈을 유지하되 태블릿 구간(`769px`~`1376px`)에서만 해당 `br`를 숨깁니다. 또한 모바일 전까지 이미지와 텍스트는 동일한 계산 폭으로 배치되고 텍스트 영역에는 `min-width: 0`을 적용해 줄바꿈이 정상 동작하도록 유지합니다.
