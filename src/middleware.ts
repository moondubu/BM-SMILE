...
const REDIRECT_MAP: Record<string, string> = {
  "/company/index.html": "/strategy/",
  "/company/vision.html": "/strategy/",
...
export function middleware(request: NextRequest) {
  ...
  // 2. 레거시 디렉토리 Fallback (패턴 매칭)
  const legacyDirectories = ["/company/", "/business/", "/board/"]
...
// 3. 미들웨어가 실행될 경로 최적화 (레거시 경로들만 가로챔)
export const config = {
  matcher: [
    "/company/:path*",
    "/business/:path*",
